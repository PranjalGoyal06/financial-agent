...
async def evidence_triage(state: ResearchState) -> dict:
    ...
    for ticker, pack in ticker_evidence.items():
        ...
        run_logger.log_debug("evidence_triage", f"Sufficiency check for {ticker}", {"ticker": ticker, "score": result.score, "is_sufficient": result.is_sufficient, "item_count": result.item_count})
    ...
    if tasks:
        ...
        for ticker, queries in zip(tickers, results):
            if isinstance(queries, Exception):
                ...
                run_logger.log_debug("evidence_triage", f"Follow-up queries generated for {ticker}", {"ticker": ticker, "queries": queries})
